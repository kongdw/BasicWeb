$.menus = {
    initMenu: function () {
        ...
        var $div = $(".page-sidebar-menu");
        var addMenus = function (data, parent, rootPath) {
            $.each(data, function (i, m) {
                var liTemplate = '<li><a href="javascript:void(0);" data-path="{rootPath}"><i class="{icon}"></i><span class="title">{name}</span></a></li>';
        ...
        if ($div.length > 0) {
            $div.ajaxJsonUrl(url, function (data) {
                addMenus(data, $div);
            });
        }
    }
