...
var App = function () {
    ...
    var handleSidebarMenu = function () {
        ...
        var _handleAddressChange = function () {
            $.address.change(function (data) {
                console.log(data.value);
                if (data.value == "/") {
                    return;
                }
                var a = $('#nav ul > li > a[rel^="address:' + data.value + '"]');
                _handleBreadcrumbs(a.data("path"));
                _handlePageNav(a);
                //_handlePageContent(a.attr("rel"));
            });
        };
        var _handlePageNav = function (a) {
            var pageNav = $("#page-nav");
            var ul = pageNav.find('ul');
            var $a = ul.find('li > a[rel^="' + a.attr("rel") + '"]');
            if ($a.length == 0) {
                var li = '<li><a href="javascript:void(0);" rel="{url}">{name}</a></li>';
                var names = a.data("path").split(":");
                li = li.replace("{name}", names[names.length - 1])
                    .replace("{id}", a.attr("data-id"))
                    .replace("{url}", a.attr("rel"));
                $(li).appendTo(ul);
                ul.find("a").closest("li").click(function(){
                    var url = $(this).find('a').attr("rel");
                    _handlePageContent(url);
                });
            }
            _handlePageContent(a.attr("rel"));
        };
        var _handlePageContent = function (rel) {
            var url = rel.replace("address:","");
            var pageContent = $("#page-content");
            var currentContent = pageContent.find('div[data-url^="'+ url + '"]');
            if (currentContent.length > 0) {
                currentContent.show();
            } else {
                var div = "<div data-url='{url}'></div>" ;
                div = div.replace("{url}",url);
                currentContent = $(div);
                currentContent.ajaxGetUrl(url).appendTo(pageContent);
            }
            pageContent.find("> div").not(currentContent).hide();
        };
        var _handleBreadcrumbs = function (path) {
            var breadcrumbs = $('#breadcrumbs');
            if (breadcrumbs.length > 0) {
                breadcrumbs.html('');
                var lis = path.split(":");
                $.each(lis, function (i, item) {
                    var liTemplate = '<li><a href="javascript:void(0);">{name}</a></li>';
                    console.log(i + item);
                    liTemplate = liTemplate.replace("{name}", item);
                    if (i == 0) {
                        $(liTemplate).prepend('<i class="icon-home"></i>').appendTo(breadcrumbs);
                    } else {
                        $(liTemplate).appendTo(breadcrumbs);
                    }
                });
    ...
    return {
        ...
        blockUI: function (el, centerY) {
            var el = $(el);
            el.block({
                message: '<img src="../img/ajax-loading.gif" alt="">',
