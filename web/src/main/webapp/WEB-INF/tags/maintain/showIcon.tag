...
<%@ attribute name="icon" type="k0n9.maintain.icon.entity.Icon" required="false" description="当前图标" %>
...
<c:choose>
    ...
    <c:when test="${not empty icon.imgSrc}">
        <%--<k:showAttachment filename="${icon.imgSrc}" showImage="true" width="${icon.width}" height="${icon.height}"/>--%>
        <%--&nbsp;&nbsp;--%>
        <i class="${icon.identity}" title="生成后的图标"></i>
    </c:when>
    <c:when test="${not empty icon.spriteSrc}">
        <%--<k:showAttachment filename="${icon.spriteSrc}" showImage="true" width="${icon.width}" height="${icon.height}" left="${icon.left}" top="${icon.top}"/>--%>
        <%--&nbsp;&nbsp;--%>
        <i class="${icon.identity}" title="生成后的图标"></i>
    </c:when>
</c:choose>
