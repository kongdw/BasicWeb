    ...
    <div> 第
        <c:choose>
            <c:when test="${page.firstPage}">
                <a href="#" class="btn btn-sm default disabled" title="首页"><i class="fa fa-angle-double-left"></i></a>
                <a href="#" class="btn btn-sm default prev disabled" title="上一页"><i class="fa fa-angle-left"></i></a>
            </c:when>
            <c:otherwise>
                <a href="#" class="btn btn-sm default" onclick="$.table.turnPage('${pageSize}', 1, this);" title="首页"><i class="fa fa-angle-double-left"></i></a>
                <a href="#" class="btn btn-sm default prev" onclick="$.table.turnPage('${pageSize}', ${current - 1}, this);" title="上一页"><i class="fa fa-angle-left"></i></a>
            </c:otherwise>
        </c:choose>
        <form style="display: inline" class="pageField">
            <input type="text"
                   class="validate[required,custom[integer],min[1],max[${page.totalPages}]] pagination-panel-input form-control input-mini input-inline input-sm"
                   value="${current}"
                   onblur="$('.pageField').validationEngine('validate') && $.table.turnPage('${pageSize}', $(this).val(), this)"/>
        </form>
        <c:choose>
            <c:when test="${page.lastPage}">
                <a href="#" class="btn btn-sm default next disabled" title="下一页"><i class="fa fa-angle-right"></i></a>
                <a href="#" class="btn btn-sm default next disabled" title="尾页"><i class="fa fa-angle-double-right"></i></a>
            </c:when>
            <c:otherwise>
                <a href="#" class="btn btn-sm default next" onclick="$.table.turnPage('${pageSize}', ${current + 1}, this);" title="下一页"><i class="fa fa-angle-right"></i></a>
                <a href="#" class="btn btn-sm default next" onclick="$.table.turnPage('${pageSize}', ${page.totalPages}, this);" title="尾页"><i class="fa fa-angle-double-right"></i></a>
            </c:otherwise>
        </c:choose> 页
    </div>
</div>
<div id="" style="display: inline-block;">
    <label>
        ...
        <select class="form-control input-sm input-inline" onchange="$.table.turnPage($(this).val(), ${current}, this);">
